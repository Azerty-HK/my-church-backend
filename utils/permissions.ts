import type { MemberPosition } from '../types/database';
...
export class PermissionService {
  static getPermissionsForPosition(position?: MemberPosition): Record<string, boolean> {
  ...
  static canPerformAction(memberPermissions: Record<string, boolean>, action: string): boolean {
    return memberPermissions[action] === true;
  }

  static getRestrictedPositions(): MemberPosition[] {
    return ['Lecteur', 'Secrétaire', 'Trésorier'];
  }

  static isRestrictedPosition(position?: MemberPosition): boolean {
    if (!position) return false;
    return this.getRestrictedPositions().includes(position);
  }

  static validatePositionCreation(position?: MemberPosition, creatorPosition?: MemberPosition): ValidationResult {
    if (!position) return { isValid: true };
  ...
  static getPositionHierarchy(): Record<MemberPosition, number> {
    return {
      'Pasteur': 10,
      'Trésorier': 8,
      'Secrétaire': 7,
  ...
  static canManagePosition(managerPosition?: MemberPosition, targetPosition?: MemberPosition): boolean {
    if (!managerPosition || !targetPosition) return false;
