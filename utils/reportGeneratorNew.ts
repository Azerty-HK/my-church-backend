...
import { FileSystem } from 'expo-file-system'; // NOUVELLE API
import { formatCurrency, formatAmount, getCurrencySymbol, getTotalInMainCurrency } from './currency';
...
export class ReportGeneratorService {
  ...
  static async generateAdvancedReportV2(data: AdvancedReportData): Promise<string> {
    const html = this.generateHTML(data);
  ...
  static async exportToPDF(data: AdvancedReportData): Promise<string> {
    try {
  ...
  private static generateHTML(data: AdvancedReportData): string {
    ...
    const totalCaisseMainCurrency = getTotalInMainCurrency({
      FC: caisseFC,
      USD: caisseUSD,
      EURO: caisseEURO
    }, data.church.currency);

    const totalBanqueMainCurrency = getTotalInMainCurrency({
      FC: banqueFC,
      USD: banqueUSD,
      EURO: banqueEURO
    }, data.church.currency);
    ...
    const totalRevenueMainCurrency = getTotalInMainCurrency(data.stats.totalRevenueByCurrency, data.church.currency);
    const totalExpensesMainCurrency = getTotalInMainCurrency(data.stats.totalExpensesByCurrency, data.church.currency);
    const totalBalanceMainCurrency = getTotalInMainCurrency(data.stats.balanceByCurrency, data.church.currency);
  ...
  <div class="section">
    ...
    <div class="stats-grid">
      <div class="stat-card stat-card-main">
        ...
        <div class="stat-value stat-main">
          ${formatCurrency(totalRevenueMainCurrency, data.church.currency)}
        </div>
      </div>
      <div class="stat-card stat-card-main">
        <div class="stat-label">Dépenses totales (${data.church.currency})</div>
        <div class="stat-value stat-main stat-negative">
          ${formatCurrency(totalExpensesMainCurrency, data.church.currency)}
        </div>
      </div>
      <div class="stat-card stat-card-main">
        ...
        <div class="stat-value stat-main ${totalBalanceMainCurrency >= 0 ? 'stat-positive' : 'stat-negative'}">
          ${formatCurrency(totalBalanceMainCurrency, data.church.currency)}
        </div>
      </div>
    </div>
      ...
      <div class="stats-grid">
        ...
        <div class="stat-card stat-card-expense-fc">
          ...
          <div class="stat-value stat-negative">
            ${formatCurrency(expensesByCurrency.FC, 'FC')}
          </div>
        </div>
        ` : ''}
        ...
        <div class="stat-card stat-card-expense-usd">
          ...
          <div class="stat-value stat-negative">
            ${formatCurrency(expensesByCurrency.USD, 'USD')}
          </div>
        </div>
        ` : ''}
        ...
        <div class="stat-card stat-card-expense-euro">
          ...
          <div class="stat-value stat-negative">
            ${formatCurrency(expensesByCurrency.EURO, 'EURO')}
          </div>
        </div>
        ` : ''}
    ...
    <div class="stats-grid">
      <div class="stat-card stat-card-fc">
        ...
        <div class="stat-value stat-positive">
          ${formatCurrency(data.stats.totalRevenueByCurrency.FC, 'FC')}
        </div>
      </div>
      <div class="stat-card stat-card-usd">
        <div class="stat-label">Comptes rendus USD</div>
        <div class="stat-value stat-positive">
          ${formatCurrency(data.stats.totalRevenueByCurrency.USD, 'USD')}
        </div>
      </div>
      <div class="stat-card stat-card-euro">
        <div class="stat-label">Comptes rendus EURO</div>
        <div class="stat-value stat-positive">
          ${formatCurrency(data.stats.totalRevenueByCurrency.EURO, 'EURO')}
        </div>
      </div>
    </div>
    ...
    <div class="stats-grid">
      <div class="stat-card stat-card-fc">
        <div class="stat-label">Dépenses FC</div>
        <div class="stat-value stat-negative">
          ${formatCurrency(data.stats.totalExpensesByCurrency.FC, 'FC')}
        </div>
      </div>
      <div class="stat-card stat-card-usd">
        <div class="stat-label">Dépenses USD</div>
        <div class="stat-value stat-negative">
          ${formatCurrency(data.stats.totalExpensesByCurrency.USD, 'USD')}
        </div>
      </div>
      <div class="stat-card stat-card-euro">
        <div class="stat-label">Dépenses EURO</div>
        <div class="stat-value stat-negative">
          ${formatCurrency(data.stats.totalExpensesByCurrency.EURO, 'EURO')}
        </div>
      </div>
    </div>
    ...
    <div class="stats-grid">
      <div class="stat-card stat-card-fc">
        ...
        <div class="stat-value ${data.stats.balanceByCurrency.FC >= 0 ? 'stat-positive' : 'stat-negative'}">
          ${formatCurrency(data.stats.balanceByCurrency.FC, 'FC')}
        </div>
      </div>
      <div class="stat-card stat-card-usd">
        ...
        <div class="stat-value ${data.stats.balanceByCurrency.USD >= 0 ? 'stat-positive' : 'stat-negative'}">
          ${formatCurrency(data.stats.balanceByCurrency.USD, 'USD')}
        </div>
      </div>
      <div class="stat-card stat-card-euro">
        ...
        <div class="stat-value ${data.stats.balanceByCurrency.EURO >= 0 ? 'stat-positive' : 'stat-negative'}">
          ${formatCurrency(data.stats.balanceByCurrency.EURO, 'EURO')}
        </div>
      </div>
    </div>
    ...
    <div class="section main-currency-section">
      ...
      <div class="main-currency-breakdown">
        ...
        <div class="breakdown-row">
          ...
          <strong class="main-currency-text">${formatCurrency(totalCaisseMainCurrency, data.church.currency)}</strong>
        </div>
        <div class="breakdown-row">
          <span>🏦 Total Banque</span>
          <strong class="main-currency-text">${formatCurrency(totalBanqueMainCurrency, data.church.currency)}</strong>
        </div>
        <div class="breakdown-row" style="border-top: 2px solid #f39c12; font-weight: bold;">
          <span>📊 Total Général</span>
          <strong class="main-currency-text">${formatCurrency(totalCaisseMainCurrency + totalBanqueMainCurrency, data.church.currency)}</strong>
        </div>
      </div>
      ...
      <div class="currency-breakdown currency-breakdown-fc">
        ...
        <div class="breakdown-row">
          <span>💰 Caisse FC</span>
          <strong>${formatCurrency(caisseFC, 'FC')}</strong>
        </div>
        <div class="breakdown-row">
          <span>🏦 Banque FC</span>
          <strong>${formatCurrency(banqueFC, 'FC')}</strong>
        </div>
        <div class="breakdown-row" style="border-top: 2px solid #27ae60; font-weight: bold;">
          <span>📊 Total FC</span>
          <strong>${formatCurrency(caisseFC + banqueFC, 'FC')}</strong>
        </div>
      </div>
      ...
      <div class="currency-breakdown currency-breakdown-usd">
        ...
        <div class="breakdown-row">
          ...
          <strong>${formatCurrency(caisseUSD, 'USD')}</strong>
        </div>
        <div class="breakdown-row">
          <span>🏦 Banque USD</span>
          <strong>${formatCurrency(banqueUSD, 'USD')}</strong>
        </div>
        <div class="breakdown-row" style="border-top: 2px solid #2980b9; font-weight: bold;">
          <span>📊 Total USD</span>
          <strong>${formatCurrency(caisseUSD + banqueUSD, 'USD')}</strong>
        </div>
      </div>
      ` : ''}
      ...
      <div class="currency-breakdown currency-breakdown-euro">
        ...
        <div class="breakdown-row">
          ...
          <strong>${formatCurrency(caisseEURO, 'EURO')}</strong>
        </div>
        <div class="breakdown-row">
          <span>🏦 Banque EURO</span>
          <strong>${formatCurrency(banqueEURO, 'EURO')}</strong>
        </div>
        <div class="breakdown-row" style="border-top: 2px solid #8e44ad; font-weight: bold;">
          <span>📊 Total EURO</span>
          <strong>${formatCurrency(caisseEURO + banqueEURO, 'EURO')}</strong>
        </div>
      </div>
      ` : ''}
    </div>
          ...
          html += `
            <tr>
              ...
              <td class="${amountClass}">
                ${amountSign}${formatCurrency(Number(transaction.amount), transaction.currency)}
              </td>
            </tr>
          `;
        });
    ...
    <div class="section">
      ...
      <h3>Francs Congolais (FC) <span class="currency-tag tag-fc">${formatCurrency(revenuesByCurrency.FC, 'FC')}</span></h3>
        ...
        <tbody>
          ${data.dailyReports.filter(r => r.currency === 'FC').map(report => `
            <tr>
              ...
              <td class="amount-positive">+${formatCurrency(report.amount, 'FC')}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
      ...
      <h3>Dollars US (USD) <span class="currency-tag tag-usd">${formatCurrency(revenuesByCurrency.USD, 'USD')}</span></h3>
        ...
        <tbody>
          ${data.dailyReports.filter(r => r.currency === 'USD').map(report => `
            <tr>
              ...
              <td>${report.recorded_by}</td>
              <td>${report.payment_method === 'cash' ? 'Caisse' : 'Banque'}</td>
              <td class="amount-positive">+${formatCurrency(report.amount, 'USD')}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
      ...
      <h3>Euros (EURO) <span class="currency-tag tag-euro">${formatCurrency(revenuesByCurrency.EURO, 'EURO')}</span></h3>
        ...
        <tbody>
          ${data.dailyReports.filter(r => r.currency === 'EURO').map(report => `
            <tr>
              ...
              <td>${report.recorded_by}</td>
              <td>${report.payment_method === 'cash' ? 'Caisse' : 'Banque'}</td>
              <td class="amount-positive">+${formatCurrency(report.amount, 'EURO')}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
    ...
    <div class="section">
      ...
      <!-- FC -->
      ${expensesByCurrency.FC > 0 ? `
      <h3>Francs Congolais (FC) <span class="currency-tag tag-expense">${formatCurrency(expensesByCurrency.FC, 'FC')}</span></h3>
        ...
        <tbody>
          ${data.expenses.filter(e => e.currency === 'FC').map(expense => `
            <tr>
              ...
              <td class="amount-negative">-${formatCurrency(expense.amount, 'FC')}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
      ...
      <!-- USD -->
      ${expensesByCurrency.USD > 0 ? `
      <h3>Dollars US (USD) <span class="currency-tag tag-expense">${formatCurrency(expensesByCurrency.USD, 'USD')}</span></h3>
      <table class="expense-table">
        <thead>
          <tr>
            <th>Date</th>
        ...
        <tbody>
          ${data.expenses.filter(e => e.currency === 'USD').map(expense => `
            <tr>
              ...
              <td>${expense.recorded_by}</td>
              <td>${expense.payment_method === 'cash' ? 'Caisse' : 'Banque'}</td>
              <td class="amount-negative">-${formatCurrency(expense.amount, 'USD')}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
      ...
      <!-- EURO -->
      ${expensesByCurrency.EURO > 0 ? `
      <h3>Euros (EURO) <span class="currency-tag tag-expense">${formatCurrency(expensesByCurrency.EURO, 'EURO')}</span></h3>
      <table class="expense-table">
        <thead>
          <tr>
            <th>Date</th>
        ...
        <tbody>
          ${data.expenses.filter(e => e.currency === 'EURO').map(expense => `
            <tr>
              ...
              <td>${expense.recorded_by}</td>
              <td>${expense.payment_method === 'cash' ? 'Caisse' : 'Banque'}</td>
              <td class="amount-negative">-${formatCurrency(expense.amount, 'EURO')}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
        ...
        ${Object.entries(data.stats.salaryByCurrency)
          ...
          .map(([currency, amount]) => `
            <div class="stat-card stat-card-${currency.toLowerCase()}">
              ...
              <div class="stat-value">${formatCurrency(amount, currency)}</div>
            </div>
          `).join('')}
      </div>
      ${data.church.currency && data.stats.salaryByCurrency[data.church.currency as keyof typeof data.stats.salaryByCurrency] ? `
        <div class="main-currency-highlight">
          ⭐ Total salaires en ${data.church.currency}: 
          ${formatCurrency(data.stats.salaryByCurrency[data.church.currency as keyof typeof data.stats.salaryByCurrency], data.church.currency)}
        </div>
      ` : ''}
    </div>
    ` : ''}
