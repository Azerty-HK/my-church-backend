import type { MemberPosition, Department, MemberType } from '../types/database.js';

export const APP_NAME = 'My Church Management System';
export const APP_VERSION = '4.3.0'; // Version avec gestion cartes et paiement
export const APP_VERSION_CODE = 430;
...
export const RESTRICTED_POSITIONS: MemberPosition[] = [
  'Pasteur',
  'Trésorier',
  'Secrétaire',
  'Lecteur'
...
// Default MySQL configuration (can be overridden via env vars)
export const MYSQL_CONFIG = {
  host: process.env.MYSQL_HOST || 'localhost',        // localhost si local, ou IP/hostname serveur
  port: Number(process.env.MYSQL_PORT) || 3306,       // port MySQL par défaut
  user: process.env.MYSQL_USER || 'mychurch_app',     // l'utilisateur MySQL créé
  password: process.env.MYSQL_PASSWORD || 'MyChurch2026!', // mot de passe sécurisé
  database: process.env.MYSQL_DATABASE || 'mychurch', // nom de la base
...
export const CODE_GENERATORS = {
  ...
  QR_CODE: (memberId: string) => `QR-MEM-${memberId}-${Date.now().toString(36).toUpperCase()}`
};
...
export const SUBSCRIPTION_CONFIG = {
  ...
  MESSAGES: {
    TRIAL_ACTIVE: (days: number) => `Essai gratuit - ${days} jour${days > 1 ? 's' : ''} restant${days > 1 ? 's' : ''}`,
    SUBSCRIPTION_ACTIVE: (days: number) => `Abonnement actif - ${days} jour${days > 1 ? 's' : ''} restant${days > 1 ? 's' : ''}`,
    PAYMENT_DURING_TRIAL: (date: string) => `Paiement validé ✅. Votre abonnement commencera à la fin de votre essai gratuit le ${date}.`,
    PAYMENT_AFTER_TRIAL: (duration: number) => `Votre abonnement commence maintenant pour ${duration} jours.`,
    PAYMENT_NEW_TRIAL: (date: string) => `Paiement validé ✅. Votre essai gratuit de 30 jours commence maintenant. Votre abonnement débutera le ${date}.`,
  },
...
// ==================== PAIEMENTS SÉCURISÉS ====================
export const SECURE_PAYMENT_CONFIG = {
...
export const SUBSCRIPTION_CALCULATOR = {
  ...
  getSubscriptionPrice: (type: 'monthly' | 'yearly') => {
    const plan = SUBSCRIPTION_CONFIG.PLANS.find(p => p.type === type);
    return plan?.price || 0;
  },
  
  // Obtenir les détails du plan
  getPlanDetails: (type: 'trial' | 'monthly' | 'yearly') => {
    return SUBSCRIPTION_CONFIG.PLANS.find(p => p.type === type);
  }
};

// Exécuter le calcul d'économie
export const YEARLY_SAVINGS_CALCULATION = SUBSCRIPTION_CALCULATOR.calculateYearlySavings(
  SUBSCRIPTION_CONFIG.MONTHLY_PRICE,
  SUBSCRIPTION_CONFIG.YEARLY_PRICE
);
...
export let DB_PROVIDER: DbProvider = 'mysql';

export function setDbProvider(provider: DbProvider) {
...
export default {
  ...
  FILTER_STATUS,
  SUBSCRIPTION_CONFIG,
  SECURE_PAYMENT_CONFIG,
  DEVELOPER_INFO,
  VERSION_INFO,
  SUBSCRIPTION_CALCULATOR,
  YEARLY_SAVINGS_CALCULATION
};
