...
class SubscriptionService {
    static async checkAccess(churchId) {
        try {
            return await database_1.DatabaseService.checkSubscriptionStatus(churchId);
        }
        catch (error) {
            return false;
        }
    }
    static async processPayment(churchId, amount, paymentMethod, phoneNumber) {
        try {
            console.log('💳 Début paiement My Church:', { amount, paymentMethod, phoneNumber });
            // Créer la transaction
            const transaction = await database_1.DatabaseService.createPaymentTransaction({
                church_id: churchId,
                amount,
    ...
    static async simulateRealisticPayment(transaction) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                await database_1.DatabaseService.updatePaymentTransaction(transaction.id, {
                    status: 'completed',
    ...
    static async renewSubscription(churchId, type) {
        try {
            ...
            const updatedChurch = await database_1.DatabaseService.updateChurch(churchId, updates);
    ...
    static getSubscriptionPrice(type) {
        const plan = this.PLANS.find(p => p.type === type);
        return plan?.price || 0;
    }
    static getRemainingDays(endDate) {
        const end = new Date(endDate);
        const now = new Date();
        const diffTime = end.getTime() - now.getTime();
        return Math.ceil(diffTime / (1000 * 60 * 60 * 24));
    }
    static getSubscriptionStatus(church) {
        if (!church) {
            return {
                isActive: false,
                type: 'none',
...
SubscriptionService.PAYMENT_METHODS = [
    {
        id: 'mpesa',
        name: 'M-Pesa',
        type: 'mobile',
